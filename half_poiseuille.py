...
Kxx, Kxy, Kyx, Kyy, K, M, Mr, MLump, Gx, Gy, Gx1r, Gy1r, polynomial_order = assembly.AxiElement2D(polynomial_option, GL, npoints, nelem, IEN, x, y, gausspoints)
...
if polynomial_option == 1:
 ...
 condition_xvelocity = benchmark_problems.axiHalf_Poiseuille(nphysical,npoints,x,y)
 ...
 condition_yvelocity = benchmark_problems.axiHalf_Poiseuille(nphysical,npoints,x,y)
 ...
 streamfunction_LHS0 = sps.lil_matrix.copy(Kxx) + sps.lil_matrix.copy(Kyy) + sps.lil_matrix.copy(Gy1r)
 condition_streamfunction = benchmark_problems.axiHalf_Poiseuille(nphysical,npoints,x,y)
...
for t in tqdm(range(0, nt)):
 ...
 vorticity_LHS = ((np.copy(M)/dt) + (1.0/Re)*np.copy(K) - (1.0/Re)*np.copy(Gy1r))
